...
export PATH=/opt/nvim-linux-x86_64/bin:/home/emalfinet/.local/bin:/home/emalfinet/.nvm/versions/node/v22.17.0/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games:/usr/local/games:/snap/bin
...
function theme_dev {
  shopify theme dev --store="$1" --theme-editor-sync
}
export theme_dev
...
function alfinet {
    ...
    if [ "$open_browser" = true ]; then
        firefox http://127.0.0.1:9292 || return
    fi
    ...
    if [ "$call_theme_dev" = true ]; then
        theme_dev "$theme_store"
    fi
}
export alfinet
